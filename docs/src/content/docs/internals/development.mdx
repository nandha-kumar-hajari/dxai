...
- **Data track** — `src/registry/data/*.json`. Push to `main`; users pick it up via the auto-refresh TTL (7 days) or `dxai update` instantly. **No version bump.** Use for new/changed servers, skills, tools, description edits, `stale` markers, pin bumps.
