...
Pass `--json` to get machine-readable output for piping into other tools. See [Non-interactive / CI](/dxai/guide/non-interactive/) for the full flag list.
...
- [Non-interactive / CI](/dxai/guide/non-interactive/) — flags, env vars, JSON output, dry-run.
- [Profiles](/dxai/guide/profiles/) — save and apply reusable setups.
- [Introspection](/dxai/guide/introspection/) — `list`, `status`, `doctor` in depth.
- [CLI Reference](/dxai/reference/commands/) — every subcommand and flag.
